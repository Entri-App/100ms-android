...
class LiveSessionAnalyticsUseCase(
    ...
    fun onNetworkQualityUpdate(quality: Int) {
        ...
        if (quality != lastRecordedQuality) {
            val formattedCurrentTime = convertDateToAnalytics(currentTime) ?: "$currentTime"
            metrics.networkTimeline[formattedCurrentTime] = quality
    ...
    fun finishSession(exitReason: String): String {
        ...
        val allQualities = metrics.networkTimeline.values.filter { it >= 0 }
